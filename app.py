...
def generate_plots(N, mu, sigma2, S):
    ...
    Y = mu + np.random.normal(0, np.sqrt(sigma2), N)  # Generate Y with normal error
    ...
    plt.plot(X, model.predict(X.reshape(-1, 1)), color="red", label=f"Fitted Line: y = {intercept:.2f} + {slope:.2f}x")
    plt.xlabel("X")
    plt.ylabel("Y")
